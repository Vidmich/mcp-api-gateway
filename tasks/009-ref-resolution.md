...
- [ ] A fixture with a self-referential schema (a tree node) resolves without recursion errors.
- [ ] Two mutually recursive schemas resolve and are cut at the documented depth.
- [ ] An external `$ref` produces a warning and a permissive schema rather than an exception.
- [ ] A ref to a missing pointer is reported as a parse error naming the pointer.
