...
#: The width of ``servers.tool_prefix`` (spec §4).
MAX_SLUG: Final = 100
...
def server_slug(name: str) -> str:
    ...
    The default for ``tool_prefix`` (spec §4), which is why it lives here rather
    than with the wizard: the prefix leads every tool name this server
    publishes, so the rule that turns a name into one belongs beside the rules
    that turn the rest of a name into the rest.
    ...
    """
    # Not :func:`_fit`: a prefix that ran long is simply cut, because the digest
    # that keeps a *tool* name unique has nothing to be unique against here —
    # the names built from the prefix are what get checked, and they are
