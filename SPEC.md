...
- **`/ui/servers/{id}`** — detail page. Same operation table plus status filters (`new`, `changed`, `removed`), inline editing of tool name and description, per-operation select toggles, and the server's own settings (name, tool prefix, base URL, API credentials, spec fetch auth, auto-refresh checkbox, and the optional rate limit — two boxes that are one setting, both empty for no cap, taking effect on the next call with no restart). Both credential sets are write-only in the UI: the current value is never rendered back, only "set" / "not set" with a Replace action. Its summary repeats the list's **Status** — the same three counts, from the same template — so an operator arriving from that table reads the numbers they just saw rather than a second wording of them; whether the server is on is said here by the badge beside the page title, which this page has the room for and a scanned row does not.
